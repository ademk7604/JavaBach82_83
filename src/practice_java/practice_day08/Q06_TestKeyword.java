...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        for (int i = 0; i < answers.length ; i++) {
            int sayac=0; // herbir ogrencinin dogru cevabini sayacagi icn, sayac buraya konur
            for (int j = 0; j < answers[i].length ; j++) {
                if (answers[i][j]==keys[j]){ // cevap anahtari ile ogrenvi cevaplari karsilastiriliyor
                    sayac++;
                }
