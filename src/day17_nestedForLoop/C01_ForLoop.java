...
public class C01_ForLoop {
    ...
    public static void tekrarsizYap(String input) {
        ...
        for (int i = 1; i <islenecekKelime.length() ; i++) {
            if (!benzersizInput.contains(islenecekKelime.substring(i,i+1))){// (!) isl.kelime ben.input ta yoksa
                System.out.print(", "+ islenecekKelime.substring(i,i+1)); // basina (,) gelmemesi icin yukarida ilk degeri girdik
                benzersizInput+=islenecekKelime.substring(i,i+1);         // bu siradaki harf
            }
        }
        System.out.println("");
